...
public class CommonHelper {
   ...
   public static MeteoItem getMeteoItemFromDataPoint(FIODataPoint dataPoint,
      ...
      if (daily) {
         result = new MeteoItem(getIconResourceIdFromString(icon), dataPoint
               .temperatureMin(), dataPoint.temperatureMax(), dataPoint
               .humidity(), dataPoint.precipProbability(), -1, -1, -1);

         // hourly data points only include temp
      } else {
         result = new MeteoItem(getIconResourceIdFromString(icon), dataPoint
               .temperature(), dataPoint.temperature(), dataPoint.humidity(),
               dataPoint.precipProbability(), -1, -1, -1);
      }
