...
public class WebRenderActivity extends Activity implements HttpResultCallable {
			...
			public void run() {
				try {
					...
					case INSTRUCTOR:
						...
						break;
					case CAMPIONI:
						url = new URL(CAMPIONI_URI);
						break;
					}

					// open an http request towards for the requested info
					// use Request.SCUDERIA as a generic request type, as the
					// return parameter is not used
	...
	public void resultAvailable(Request request, final String[] result,
		...
		if ((result != null) && (result[0] != null))
			runOnUiThread(new Runnable() {
				@Override
				public void run() {
					container.loadDataWithBaseURL(null, getAvailBtn(result[1]),
	...
	private String getAvailBtn(String response) {
		...
		if (result.indexOf("\"/pdf") != -1)
			result = result.replace("\"/pdf/",
					"\"http://www.scuolascilimone.com/pdf/");
	...
	private static final String CAMPIONI_URI = "http://www.scuolascilimone.com/it/area-riservata/news/latest/saranno-campioni";
}
